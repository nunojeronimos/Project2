    ...
    try:
        ...
        if picture_data:
            ...
            for blob in bucket.list_blobs(prefix="user_"):  # Iterate through user directories
                ...
                original_image_data = blob.blob(f"{user_name}/{user_name}.jpg").download_as_bytes()
                augmented_images = []
                for i in range(5):  # Change the number of augmented images as needed
                    augmented_image_data = blob.blob(f"{user_name}/augmented_images/{user_name}_augmented_{i}.jpg").download_as_bytes()
